...
*(updated: the current release locates both the data file and its debug file relative to the HVE installation directory, which it derives from the `HVESYSFILES` environment variable — the portion of that path preceding `supportFiles\sys`. The file name embedded in the event name is appended directly to that HVE directory path, so the data file must reside in the HVE program directory. Note also that the data file name is limited to 30 characters, so keep it short.)*
...
3. Click on the (+) sign to add an event. In the Event Name field enter `File:FILENAME`, where *FILENAME* is the title of the motion data file.  Figure 2 shows a sample Event Information dialog window. *(updated: the file name is taken from the event name by skipping exactly the five characters of the `File:` prefix, so the event name must begin with `File:` followed immediately by the data file name.)*
...
*(updated: the 2004 manual calls this debug file `ReadDataFileDebug.prn` in `C:\Program Files\HVE`. The current release writes it as `ReadDataDebugFile.prn` in the HVE program directory derived from the `HVESYSFILES` environment variable. The debug file echoes the expected basic file format, the header values as they are read, the number of data lines expected per time increment, and the ending time and time increment.)*
