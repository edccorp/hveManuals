...
The EDSVS engine does not present or use a Calculation Options dialog, and it reads no calculation-option values from the event. The one parameter historically exposed here — Roll Couple Distribution — is now taken from the vehicle's suspension data instead.
...
For EDSVS this value is **not** entered through a Calculation Options dialog. It is set on the vehicle editor's suspension screen, as the vehicle's **Roll Couple Distribution** suspension setting, and read from there by the physics engine.

Within the physics model, this parameter is the front-axle lateral load transfer coefficient $\gamma_1$, with the rear axle taking $\gamma_2 = 1 - \gamma_1$, which apportion the lateral weight shift used to compute the individual tire normal loads during cornering.

Note: An event still carries a legacy calculation-options value for this parameter (default 0.55), but the current EDSVS engine does not read it. The roll couple distribution used by the simulation always follows the vehicle editor's suspension setting.
